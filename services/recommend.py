...
class RecommendationService:
   ...
   # =====================================================================
   # PUBLIC API - RECOMMENDATION ENTRYPOINT
   # =====================================================================
   def recommend(
      self,
      user_id: int,
      ...
      """
      print(f"Generating recommendations for user_id={user_id}, top_k={top_k}")
      try:
         user_profile = self._get_user_profile(user_id)
         print(f"User profile for {user_id}: {user_profile}")

         user_interactions = self._get_user_interactions(user_id)
         print(f"User interactions for {user_id}: {user_interactions}")
   ...
   def _generate_candidates(
      ...
      # CF candidates
      for cand in self._generate_cf_candidates(
            user_id=user_id, limit=top_k * 2):
   ...
   def _generate_cf_candidates(
         self, user_id: int, limit: int) -> List[Dict[str, Any]]:
      """Sinh candidate từ mô hình CF (ALS); trả [] nếu không khả dụng."""
      try:
         if (not self.cf_model or not self.cf_user_id_to_index or
               not self.cf_index_to_item_id):
            return []
   ...
   def _generate_content_candidates(
      ...
      for hit in hits:
         # Handle Milvus hit object format
         job_id = None
         score = 0.0

         if hasattr(hit, "entity"):
            job_id = (hit.entity.get("id") if hasattr(hit.entity, "get")
                    else getattr(hit.entity, "id", None))
   ...
   def _rank_candidates(
      ...
      for candidate in candidates:
         ...
         if (popular_score is not None and cf_score is None and
               content_score is None):
   ...
   def _exploration_bonus(self, job_id: int) -> float:
      ...
      noise = (hashed & 0xFFFF) / 0xFFFF
      return 0.03 + 0.02 * noise

   def _collect_seen_job_ids(
         self, interactions: Optional[Dict[str, Any]]) -> set:
      ...
      for entries in interactions.values():
         if isinstance(entries, dict):
            seen.update(int(jid) for jid in entries.keys()
                     if str(jid).isdigit())
   ...
   def _calculate_user_vector(
      ...
      This method combines long-term (profile) and short-term (interactions)
      vectors using adaptive weighting based on interaction count:
   ...
   def _calculate_long_term_user_vector(
      ...
      (skills, education, location, preferences). This vector is saved to
      Milvus.
      """
      ...
      if user_id is not None:
         try:
            ...
            print(
               f"Warning: Failed to upsert long-term user vector for "
               f"user {user_id}: {e}")
   ...
   def _calculate_short_term_user_vector(
      ...
      This represents the user's current interests. Cached in Redis.
      """
      ...
      if self.redis_client:
         try:
            ...
            print(
               f"Warning: Failed to cache short-term user vector in "
               f"Redis: {e}")
   ...
   def invalidate_short_term_cache(self, user_id: int) -> None:
      ...
      if self.redis_client:
         try:
            ...
            print(
               f"Warning: Failed to invalidate short-term cache for "
               f"user {user_id}: {e}")
   ...
   def _compute_behavior_dense(
      ...
      for raw_key, entries in interactions.items():
         ...
         if (key_upper not in allowed_keys or
               key_upper not in INTERACTION_WEIGHTS):
            continue
   ...
   def _build_profile_text(
      ...
      if isinstance(preferences, dict):
         ...
         if "relocation" in preferences:
            parts.append(
               f"Open to relocation: "
               f"{self._to_text(preferences.get('relocation'))}")
      ...
      if user_interactions:
         ...
         if insights.get('preferred_skills'):
            parts.append(
               f"Interested in: "
               f"{', '.join(insights['preferred_skills'][:5])}")
   ...
   def _extract_interaction_insights(
         self, interactions: Dict[str, Any]) -> Dict[str, List[str]]:
   ...
   def _get_jobs_metadata(self, job_ids: List[str]) -> List[Dict]:
      """Get job metadata by job IDs"""
      if not job_ids:
         return []

      try:
         api_url = (f"{Config.CANDIDATE_API_BASE_URL}"
                  f"/api/jobs/public/metadata")
   ...
   def _hydrate_candidate_metadata(
         self, candidate_map: Dict[int, Dict[str, Any]]) -> None:
      """Bổ sung metadata còn thiếu cho candidate (nếu có API hỗ trợ)."""
      missing_ids = [
         job_id for job_id, data in candidate_map.items()
   ...
   def _start_interaction_consumer(self):
      ...
      if not self.redis_client:
         print("Warning: Redis not available, skipping interaction stream "
              "consumer")
         return
   ...
   def _process_interaction_stream(self):
      ...
      if not self.redis_client:
         print("Warning: Redis not available, cannot process interaction "
              "stream")
         return
      ...
      try:
         ...
         messages = self.redis_client.xread(
            {stream_name: "$"}, count=10, block=1000)

         # print(f"[STREAM] Read from '{stream_name}': "
         #      f"{len(messages)} batch(es)")
         for stream, msgs in messages:
            for msg_id, fields in msgs:
               print(f"[STREAM] RAW MESSAGE id={msg_id}, "
                    f"fields={fields}")
               try:
                  self._handle_interaction_event(fields)
                  self.redis_client.xack(
                     stream_name, "recommend-service-group", msg_id)
               except Exception as e:
                  print(
   ...
   def _handle_interaction_event(self, fields: Dict[str, bytes]):
      ...
      try:
         ...
         for key, value in fields.items():
            ...
            value_str = (value.decode() if isinstance(value, bytes)
                      else value)
         ...
         # Extract fields
         account_id = (event_data.get("accountId") or
                    event_data.get("account_id"))
         job_id = event_data.get("jobId") or event_data.get("job_id")
         event_type = (event_data.get("eventType") or
                    event_data.get("event_type"))
         occurred_at = (event_data.get("occurredAt") or
   ...
   def _update_user_interactions_cache(
      ...
      try:
         ...
         self.redis_client.setex(
            cache_key,
            7 * 24 * 3600,
            json.dumps({k: dict(v) if isinstance(v, dict) else v
   ...
   def _get_user_profile(self, user_id: int) -> Optional[Dict[str, Any]]:
      ...
      try:
         api_url = (f"{Config.CANDIDATE_API_BASE_URL}"
                  f"/api/candidates/profile/{user_id}")
            ...
            if data.get("code") == 1000 and data.get("data"):
               ...
               if "educations" in profile_data:
                  profile_data["education"] = profile_data.pop(
                     "educations")
               print(f"Profile data for user {user_id}: {profile_data}")
               return profile_data
   ...
   def _get_popular_jobs(
      ...
      """Lấy danh sách job phổ biến từ API."""
      if not limit:
         return []

      try:
         api_url = f"{Config.CANDIDATE_API_BASE_URL}/api/jobs/public/popular"
         payload = {"limit": limit}
         response = requests.get(api_url, json=payload, timeout=5)
         if response.status_code == 200:
            data = response.json()
            if data.get("code") == 1000 and data.get("data"):
               return data["data"]
