   ...
   # ================== Recommendation Entrypoint ==================
    ...
    def _generate_cf_candidates(
        ...
        try:
            ...
            # Use model.recommend with user_items=None not allowed; instead
            # pass empty csr row.
    ...
    def _get_jobs_metadata(self, job_ids: List[str]) -> List[Dict]:
        ...
        try:
            api_url = f"{Config.CANDIDATE_API_BASE_URL}/api/job/metadata"
    ...
    def _exp_time_decay(
        ...
        try:
            ...
            return math.exp(-math.log(2) *
                            (delta_days / float(half_life_days)))
        except Exception:
            return 1.0
    ...
    def _process_interaction_stream(self):
        ...
        if not self.redis_client:
            print("Warning: Redis not available, cannot process interaction stream")
            return
        ...
        try:
            ...
            for stream, msgs in messages:
                for msg_id, fields in msgs:
                    print(f"[STREAM] RAW MESSAGE id={msg_id}, fields={fields}")
                    try:
                        # Xử lý event
    ...
    def _get_user_profile(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            api_url = f"{
                Config.CANDIDATE_API_BASE_URL}/api/candidates/profile/{user_id}"
